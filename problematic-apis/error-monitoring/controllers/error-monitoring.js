...
module.exports = {
  ...
  async getStats(ctx) {
    try {
      // Mock error stats since error-handler was TypeScript
      const stats = {
        total: 0,
        byType: {},
        bySeverity: {},
      ...
      // Mock performance stats since performance-monitor was TypeScript
  ...
  async getErrorDetails(ctx) {
    try {
      ...
      const errorDetails = {
        ...
        recentOccurrences: Array.from({ length: Math.min(parseInt(limit), 50) }, (_, i) => ({
  ...
  async testError(ctx) {
    ...
    const error = testErrors[errorType] || testErrors.validation;
    throw error;
  },
