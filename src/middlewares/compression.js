/**
 * Compression Middleware - JavaScript version
...
module.exports = (config = {}, { strapi }) => {
    ...
    // Content type filter
    const defaultFilter = (contentType) => {
        if (!contentType)
            return false;
        // Check if content type is in the compressible list
        return COMPRESSIBLE_TYPES.some(type => contentType.toLowerCase().includes(type));
    };
    ...
    if (config.br?.enabled && typeof compress.br !== 'undefined') {
 ...
 */
function shouldSkipCompression(ctx, config) {
