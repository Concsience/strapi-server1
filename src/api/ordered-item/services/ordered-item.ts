...
export default factories.createCoreService('api::ordered-item.ordered-item', ({ strapi }) => ({
  /**
    ...
    try {
      ...
      orderedItems.results.forEach(item => {
        ...
        if (status in fulfillmentCounts) {
          fulfillmentCounts[status as keyof typeof fulfillmentCounts]++;
        }
   ...
   */
  async bulkUpdateFulfillment(updates: FulfillmentUpdate[]) {
    try {
      const results = [];
   ...
   */
  async createFromCartItems(cartItems: any[], orderId: string) {
    try {
      const orderedItems = [];
    ...
    try {
      ...
      return {
        ...
        summary: Object.entries(grouped).map(([paperType, items]) => ({
          ...
          totalQuantity: (items as any[]).reduce((sum, item) => sum + (item.quantity || 1), 0)
        }))
      };
