...
module.exports = factories.createCoreController('api::cart.cart', ({ strapi }) => ({
    ...
    async updateItemQuantity(ctx) {
        try {
            ...
            const updatedItem = await strapi.service('api::cart.cart').updateItemQuantity(cart.id, itemId, quantity);
            ctx.send({
                data: updatedItem,
                meta: {
    ...
    async checkout(ctx) {
        try {
            ...
            const cart = await strapi.service('api::cart.cart').getOrCreateCart(userId);
            // Perform checkout
            const order = await strapi.service('api::cart.cart').checkout(cart.id, paymentIntentId);
            ctx.send({
                data: order,
                meta: {
