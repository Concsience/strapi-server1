...
function calculateWishlistValue(artworks) {
...
function calculateEstimatedPrice(artwork) {
    const standardSize = 30 * 40; // 30x40cm
    const basePrice = (artwork.base_price_per_cm_square || 0) * standardSize;
    return Math.round(basePrice * 100) / 100;
}
function getPopularityTier(score) {
    if (score >= 100)
        return 'trending';
    if (score >= 50)
        return 'popular';
...
module.exports = factories.createCoreController('api::wishlist.wishlist', ({ strapi }) => ({
    /**
    ...
    async getMyWishlist(ctx) {
        try {
            ...
            const enhancedWishlist = {
                ...
                items: (userWishlist.arts || []).map((artwork) => ({
                    ...artwork,
    ...
    async addItem(ctx) {
        try {
            ...
            if (wishlist.results.length === 0) {
                ...
                const isAlreadyInWishlist = userWishlist.arts?.some((art) => art.documentId === artworkId || art.id === parseInt(artworkId));
                if (isAlreadyInWishlist) {
                    return ctx.conflict('Artwork is already in your wishlist');
                }
                // Add artwork to existing wishlist
                const currentArtworkIds = userWishlist.arts?.map((art) => art.documentId || art.id) || [];
    ...
    async checkItem(ctx) {
        try {
            ...
            const isInWishlist = await strapi.service('api::wishlist.wishlist').isArtworkInUserWishlist(ctx.state.user.id, artworkId);
            return ctx.send({
                data: {
    ...
    async getRecommendations(ctx) {
        try {
            ...
            const recommendations = await strapi.service('api::wishlist.wishlist').getRecommendationsForUser(ctx.state.user.id, limit);
            // Enhance recommendations with pricing
            const enhancedRecommendations = recommendations.map((artwork) => ({
                ...artwork,
                estimatedPrice: calculateEstimatedPrice(artwork),
                popularityTier: getPopularityTier(artwork.popularityscore || 0)
            }));
