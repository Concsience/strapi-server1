...
interface AddressUpdateData extends Partial<AddressCreateData> {
...
interface AddressValidationResult {
  isValid: boolean;
  errors: string[];
  normalizedData?: Partial<AddressCreateData>;
}

interface AddressStats {
  totalAddresses: number;
  userAddresses: Record<number, number>;
  regionDistribution: Record<string, number>;
  cityDistribution: Record<string, number>;
  mostCommonRegions: Array<{ region: string; count: number }>;
  validationStats: {
...
export default factories.createCoreService('api::address.address', ({ strapi }) => ({
  ...
  async validateAndNormalizeAddress(
    data: AddressCreateData | AddressUpdateData, 
    isUpdate: boolean = false
  ): Promise<AddressValidationResult> {
    try {
      const errors: string[] = [];
      const normalizedData: Partial<AddressCreateData> = {};
   ...
   */
  getDepartmentFromPostalCode(postalCode: string): string {
    const cleaned = postalCode.replace(/\s/g, '');
   ...
   */
  validateFrenchPhoneNumber(phone: string): { isValid: boolean; normalized?: string; error?: string } {
    if (!phone) {
  ...
  async getAddressStats(): Promise<AddressStats> {
    try {
      ...
      const userAddresses: Record<number, number> = {};
   ...
   */
  async searchByLocation(searchTerm: string, options: any = {}) {
    try {
   ...
   */
  async checkForDuplicates(addressData: AddressCreateData, userId: number) {
    try {
