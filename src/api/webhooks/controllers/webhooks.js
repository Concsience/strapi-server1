...
module.exports = {
  ...
  async list(ctx) {
    try {
      // Mock webhook list since webhook-system was TypeScript
      const webhooks = [];
  ...
  async update(ctx) {
    try {
      ...
      const existingWebhook = (webhookSystem).webhooks.get(id);
      if (!existingWebhook) {
  ...
  async getDeliveries(ctx) {
    try {
      ...
      let deliveries = webhookSystem.getDeliveryHistory(id, parseInt(limit));
   ...
   */
  async retryDelivery(ctx) {
    try {
      const { deliveryId } = ctx.params;

      // Get delivery
      const delivery = (webhookSystem).deliveries.get(deliveryId);
      if (!delivery) {
      ...
      const retryQueue = (webhookSystem).retryQueue;
  ...
  async testWebhook(ctx) {
    try {
      ...
      // Check if webhook exists
      const webhook = (webhookSystem).webhooks.get(id);
      if (!webhook) {
        ctx.throw(404, 'Webhook not found');
      }
   ...
   */
  async getRecentDeliveries(ctx) {
    try {
      const { limit = 100 } = ctx.query;

      const deliveries = webhookSystem.getRecentDeliveries(parseInt(limit));
