...
export default factories.createCoreController('api::artists-work.artists-work', ({ strapi }) => ({
   ...
   */
  async find(ctx: StrapiContext): Promise<ApiResponse<ArtistsWork[]> | ApiError> {
    try {
      const {
        page = 1,
        pageSize = 25,
   ...
   */
  async findOne(ctx: StrapiContext): Promise<ApiResponse<ArtistsWork> | ApiError> {
    try {
   ...
   */
  async popular(ctx: StrapiContext): Promise<ApiResponse<ArtistsWork[]> | ApiError> {
    try {
   ...
   */
  async calculatePrice(ctx: StrapiContext): Promise<ApiResponse<any> | ApiError> {
    try {
      const { documentId } = ctx.params;
