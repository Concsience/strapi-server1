...
module.exports = factories.createCoreService('api::artists-work.artists-work', ({ strapi }) => ({
    /**
     ...
     */
    calculateEstimatedPrice(artwork, width, height) {
     ...
     */
    async calculateDetailedPricing(artworkId, width, height, paperTypeId) {
        try {
            // Get artwork
     ...
     * Check if artwork is available for ordering
     */
    checkAvailability(artwork) {
        return !!(artwork.publishedAt &&
    ...
    async getArtworkStats() {
        try {
            ...
            if (pricingList.length > 0) {
                const totalPrice = pricingList.reduce((sum, artwork) => sum + (artwork.base_price_per_cm_square || 0), 0);
     ...
     */
    async searchArtworks(searchTerm, options = {}) {
        try {
            const { minPrice, maxPrice, artistName, minPopularity, maxPopularity, limit = 25 } = options;
            const filters = {
                $or: [
            ...
            if (minPrice || maxPrice) {
                ...
                if (minPrice)
                    filters.base_price_per_cm_square.$gte = minPrice;
                if (maxPrice)
                    filters.base_price_per_cm_square.$lte = maxPrice;
            }
            // Add popularity filters
            if (minPopularity || maxPopularity) {
                filters.popularityscore = {};
                if (minPopularity)
                    filters.popularityscore.$gte = minPopularity;
                if (maxPopularity)
                    filters.popularityscore.$lte = maxPopularity;
            }
            // Add artist filter
            if (artistName) {
     ...
     */
    async updatePopularity(artworkId, increment = 1) {
        try {
            const artwork = await strapi.entityService.findOne('api::artists-work.artists-work', artworkId);
            if (!artwork) {
                throw new Error('Artwork not found');
