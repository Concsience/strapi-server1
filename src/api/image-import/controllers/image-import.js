...
module.exports = factories.createCoreController('api::image-import.image-import', ({ strapi }) => {
    // Service functions with strapi instance available
    async function createProductSheet(metadata) {
    ...
    async function createArtwork(metadata, productsheetId) {
    ...
    async function processImageJob(imageId, job) {
        try {
    ...
    return {
        ...
        async bulkImport(ctx) {
            try {
                ...
                const { metadata, options } = ctx.request.body.data || {};
                ...
                for (let i = 0; i < metadata.length; i += batchSize) {
                    ...
                    results.push(...batchResults.map(result => result.status === 'fulfilled' ? result.value : { success: false, error: 'Promise rejected' }));
