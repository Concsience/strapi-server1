    ...
    return {
        ...
        async handleSuccessfulPayment(paymentIntent, order, orderedItems) {
            try {
                ...
                if (invoicePdfUrl) {
                    try {
                        const response = await axios_1.default.get(invoicePdfUrl, {
        ...
        async createFromCart(ctx) {
            try {
                ...
                // Calculate total
                const itemsTotal = cartItems.results.reduce((sum, item) => sum + ((item.price || 0) * (item.qty || 1)), 0);
                ...
                const orderedItems = await strapi.service('api::ordered-item.ordered-item').createFromCartItems(cartItems.results, order.documentId);
