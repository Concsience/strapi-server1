...
// Cart Item interface based on schema
interface CartItem {
  id: number;
  documentId: string;
  arttitle?: string;
  width?: number;
  height?: number;
  art?: any; // Related artwork
  cart?: any; // Related cart
  artistname?: string;
  paper_type?: any; // Related paper type
  price?: number;
  book?: any; // Related book
  book_title?: string;
  author_name?: string;
  qty?: number;
  total_price?: number;
...
interface CartItemStats {
  totalItems: number;
  totalValue: number;
  averageItemValue: number;
  mostPopularArt: any;
  mostPopularPaperType: any;
...
export default factories.createCoreController('api::cart-item.cart-item', ({ strapi }) => ({
   ...
   * POST /api/cart-items
   */
  async create(ctx: StrapiContext): Promise<ApiResponse<CartItem> | ApiError> {
    try {
      if (!hasUser(ctx)) {
        return ctx.unauthorized('Authentication required to add items to cart');
      }

      const data = ctx.request.body.data as CartItemCreateData;
      ...
      if (data.paper_type) {
        paperType = await strapi.documents('api::paper-type.paper-type').findOne({
          documentId: data.paper_type as string
        });
      }
      ...
      if (data.cart) {
        await this.updateCartTotal(data.cart as string);
      }
   ...
   */
  async update(ctx: StrapiContext): Promise<ApiResponse<CartItem> | ApiError> {
    try {
      const { id } = ctx.params;
   ...
   */
  async delete(ctx: StrapiContext): Promise<ApiResponse<{ message: string }> | ApiError> {
    try {
      const { id } = ctx.params;
   ...
   */
  async stats(ctx: StrapiContext): Promise<ApiResponse<CartItemStats> | ApiError> {
    try {
   ...
   */
  async calculatePricing(ctx: StrapiContext): Promise<ApiResponse<any> | ApiError> {
    try {
      ...
      const pricing = await strapi.service('api::cart-item.cart-item').calculateItemPricing(
        artworkId,
        width,
        height,
        paperTypeId,
   ...
   */
  async getByCart(ctx: StrapiContext): Promise<ApiResponse<any> | ApiError> {
    try {
      ...
      const result = await strapi.service('api::cart-item.cart-item').getCartItems(
        cartId,
