...
interface CartItemStats {
  ...
  quantityDistribution: Record<string, number>;
  priceRanges: {
    under50: number;
    range50to100: number;
...
export default factories.createCoreService('api::cart-item.cart-item', ({ strapi }) => ({
   ...
   */
  async getCartItems(cartId: string, options: any = {}) {
    try {
   ...
   */
  calculateSavings(cartItem: any): number {
   ...
   */
  isCustomSize(width: number, height: number): boolean {
    const standardSizes = [
  ...
  async getCartItemStats(): Promise<CartItemStats> {
    try {
      ...
      const quantityDistribution: Record<string, number> = {};
      const priceRanges = {
        under50: 0,
        range50to100: 0,
        range100to200: 0,
   ...
   */
  async validateCartItemData(data: any) {
    const errors: string[] = [];
