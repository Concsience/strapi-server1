...
import { contentHistory } from '../../../extensions/content-history/service';
...
export default {
  ...
  async searchVersions(ctx: Context) {
    try {
      ...
      if (contentType) criteria.contentType = contentType as string;
      ...
      ctx.body = {
        ...
        meta: {
          ...
          hasMore: results.length > parseInt(limit as string),
   ...
   * Helper methods
   */
  generateActivityDescription(version: any): string {
    if (!version.changes || version.changes.length === 0) {
      return 'Document created';
    }

    const changeTypes = version.changes.reduce((acc: any, change: any) => {
  ...
  summarizeChanges(changes: any[]): string[] {
    return changes
