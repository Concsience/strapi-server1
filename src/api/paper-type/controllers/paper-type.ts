...
export default factories.createCoreController('api::paper-type.paper-type', ({ strapi }) => ({
   ...
   * GET /api/paper-types
   */
  async find(ctx: StrapiContext): Promise<ApiResponse<PaperType[]> | ApiError> {
    try {
      const {
        page = 1,
        pageSize = 25,
   ...
   */
  async findOne(ctx: StrapiContext): Promise<ApiResponse<PaperType> | ApiError> {
    try {
   ...
   */
  async calculateCost(ctx: StrapiContext): Promise<ApiResponse<PriceCalculation> | ApiError> {
    try {
      const { documentId } = ctx.params;
   ...
   */
  async popular(ctx: StrapiContext): Promise<ApiResponse<PaperType[]> | ApiError> {
    try {
   ...
   */
  async stats(ctx: StrapiContext): Promise<ApiResponse<PaperTypeStats> | ApiError> {
    try {
