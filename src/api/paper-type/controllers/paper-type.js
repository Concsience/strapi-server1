...
module.exports = factories.createCoreController('api::paper-type.paper-type', ({ strapi }) => ({
    /**
    ...
    async find(ctx) {
        try {
            const { page = 1, pageSize = 25, sort = 'paper_names:asc', populate = '', filters = {}, search = '', minPrice, maxPrice, available = 'true' } = ctx.query;
            ...
            if (minPrice || maxPrice) {
                ...
                if (minPrice)
                    queryFilters.paper_price_per_cm_square.$gte = parseFloat(minPrice);
                if (maxPrice)
                    queryFilters.paper_price_per_cm_square.$lte = parseFloat(maxPrice);
            }
            const params = {
            ...
            const enhancedPaperTypes = await Promise.all(results.map(async (paperType) => {
    ...
    async popular(ctx) {
        try {
            ...
            const popularPaperTypes = await strapi.service('api::paper-type.paper-type').getPopularPaperTypes(Math.min(parseInt(limit, 10), 50));
     ...
     */
    async calculateUsageStats(paperType) {
        try {
