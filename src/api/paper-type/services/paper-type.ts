...
interface PaperTypeCreateData {
  paper_names: string;
  paper_price_per_cm_square: number;
}

interface PaperTypeUpdateData extends Partial<PaperTypeCreateData> {
...
export default factories.createCoreService('api::paper-type.paper-type', ({ strapi }) => ({
  /**
  ...
  async findWithBusinessLogic(params: any = {}) {
    try {
      ...
      const enhancedPaperTypes = await Promise.all(
        results.results.map(async (paperType) => {
   ...
   */
  async getPaperTypeRevenue(paperTypeId: string): Promise<number> {
    try {
      ...
      const totalRevenue = orderedItems.results.reduce(
   ...
   */
  calculateCostEfficiency(price: number): number {
   ...
   */
  async getPopularPaperTypes(limit: number = 10) {
    try {
      ...
      const paperTypesWithMetrics = await Promise.all(
        paperTypes.results.map(async (paperType): Promise<PopularPaperType> => {
          const usageCount = await this.getPaperTypeUsageCount(paperType.documentId);
          const revenue = await this.getPaperTypeRevenue(paperType.documentId);
   ...
   */
  async getPaperTypeStats(): Promise<PaperTypeStats> {
    try {
      // Basic counts
      ...
      const totalRevenue = orderItems.results.reduce(
        (sum, item) => sum + (parseFloat(item.total_price || '0') || 0),
        0
      );
      ...
      return {
        ...
        averagePrice: Math.round(averagePrice * 1000) / 1000, // 3 decimal places
        mostPopular,
        priceRange: {
   ...
   */
  async searchPaperTypes(searchTerm: string, options: any = {}) {
    try {
      const {
        minPrice,
        maxPrice,
      ...
      if (minPrice || maxPrice) {
        ...
        if (minPrice) filters.paper_price_per_cm_square.$gte = minPrice;
        if (maxPrice) filters.paper_price_per_cm_square.$lte = maxPrice;
      }
      ...
      if (priceCategory) {
        filteredResults = filteredResults.filter(paperType => 
          this.getPriceCategory(paperType.paper_price_per_cm_square || 0) === priceCategory
        );
      }

      // Enhance results
      const enhancedResults = await Promise.all(
        filteredResults.map(async (paperType) => {
   ...
   */
  validatePaperTypeData(data: PaperTypeCreateData | PaperTypeUpdateData): string[] {
    const errors: string[] = [];
   ...
   */
  async calculatePriceImpact(paperTypeId: string, artworkWidth: number, artworkHeight: number) {
    try {
